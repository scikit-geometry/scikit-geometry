...
def draw_vector(vector, source_x=0, source_y=0, display_range=None, vector_force=1, units='xy', angles='xy', scale_units='xy', scale=1, **kwargs):
	fig, ax = plt.gcf(), plt.gca()
	vector_x = float(vector.x())
	vector_y = float(vector.y())
	
	q = ax.quiver(np.array(source_x), np.array(source_y), np.array(vector_x*vector_force), np.array(vector_y*vector_force), units=units, angles=angles, scale_units=scale_units, scale=scale,**kwargs)
...
def draw_ray(ray, stop=0, num=100, arrow=False, visible_point=True, display_range=5, arrow_length=3, color='#1f77b4', units='xy', angles='xy', scale_units='xy', scale=1, **kwargs):
	...
	if(arrow):
		draw_vector(vector, source_x=source_x, source_y= source_y,vector_force=arrow_length, color=color)
...
def draw_direction(direction, source_x=0, source_y=0, display_range=None, units='xy', angles='xy', scale_units='xy', scale=1, **kwargs):
	fig, ax = plt.gcf(), plt.gca()
	...
	q = ax.quiver(np.array(source_x), np.array(source_y), np.array(normalized_x), np.array(normalized_y),units=units, angles=angles, scale_units=scale_units, scale=scale, **kwargs)
...
def draw_polygon(polygon=None,polygon_with_holes=None, vertices=None, plt=plt, facecolor='none', point_color='none', line_width = 2, plot_vertices=True):
	fig, ax = plt.gcf(), plt.gca()
...
def draw_voronoi(voronoi,display_range = 3, visible_point=True, edges_color='blue', sites_color='red', vertices_color ='None', finite_edges_color='blue'):
	fig, ax = plt.gcf(), plt.gca()
